...
def editCategory(categories_id):
    ...
    if editedCatalog.user_id != login_session['user_id']:
        return "<script>function myFunction() {alert('You are not authorized to edit this catagory. Please create your own restaurant in order to edit.');}</script><body onload='myFunction()''>"
    if request.method == 'POST':
...
def deleteCategroy(categories_id):
    ...
    if request.method == 'POST':
        ...
        return render_template('deletecatagory.html', categories=categoryToDelete)
...
def showMenu(categories_id):
    ...
    if 'username' not in login_session or creator.id != login_session['user_id']:
        return render_template('publicmenu.html', item=item, categories=categories, creator=creator)
    else:
        return render_template('menu.html', categories=categories, item=item, creator=creator)
     

#Create a NEW catalog item
@app.route('/catalog/<int:categories_id>/catalog_item/new/', methods=['GET', 'POST'])
def newMenuItem(categories_id):
    ...
    if request.method == 'POST':
        newItem = CatalogItem(name=request.form['name'], description=request.form['description'], price=request.form[
                                'price'], categories_id=categories_id, user_id=categories.user_id)
        session.add(newItem)
        session.commit()
...
#EDIT a catalog item
@app.route('/catalog/<int:categories_id>/catalog_item/<int:catalog_item>/edit', methods=['GET', 'POST'])
def editMenuItem(categories_id, catalog_item):
    ...
    if request.method == 'POST':
        ...
        return render_template('editmenuitem.html', categories_id=categories_id, catalog_item=catalog_item, item=editedItem)


#Delete a menu item
@app.route('/catalog/<int:categories_id>/catalog_item/<int:catalog_item>/delete', methods=['GET', 'POST'])
def deleteMenuItem(categories_id, catalog_item):
    ...
    if request.method == 'POST':
        ...
        return render_template('deletemenuitem.html', item=itemToDelete, categories_id=categories_id)
